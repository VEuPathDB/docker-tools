package SplitBamUniqueNonUnique;
use strict;
use warnings;
use Utils;
...
sub splitBamUniqueNonUnique {
    ...
    $filebase=~ s/$expDir\///;
    my $unique = "$expDir/unique_results";
    my $nonunique = "$expDir/non_unique_results";

    my $statsHash = Util::DnaSeqMetrics::runSamtoolsStats($file_to_open);
  ...
  #  print "starting on unique strand bit\n\n\n\n";
    ...
    push @filesToDelete, $unique."*bam*";
    push @filesToDelete, $nonunique."*bam*";
	push @filesToDelete, $unique."bai";
    push @filesToDelete, $nonunique."bai";
...
#    print "file running mapping stats on is $bamfile\n";
    ...
    my $coverage = Util::DnaSeqMetrics::getCoverage($directory, $bamfile);

    my $statsHash = Util::DnaSeqMetrics::runSamtoolsStats($bamfile);
